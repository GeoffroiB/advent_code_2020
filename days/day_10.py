...
class Day10(AbstractDay):
    ...
    def partOneTest(self):
        devices = sorted(list(read_ints(self.inputs_test_path)))
        built_in = np.max(devices) + 3
    ...
    def partTwoTest(self):
        ...
        def solve(prev, l_devices):
            if len(l_devices) == 1:
                ...
                result = np.sum(
                    [solve(val, l_devices[l_devices.index(val):]) for val in range(prev + 1, prev + 3 + 1) if
                     val in l_devices], dtype=np.uint64)
                known_paths[prev] = result
                return result
